...
namespace iTextDesignerWithGUI.Services
{
    ...
    public class JsonManager
    {
        ...
        public JsonManager(string jsonFilePath, AssessmentTypeWrapper assessmentTypeWrapper)
        {
            ...
            _assessmentTypeWrapper =
                assessmentTypeWrapper
                ?? throw new ArgumentNullException(nameof(assessmentTypeWrapper));
        }
        ...
        public async Task<List<object>> LoadReferenceDataAsync()
        {
            try
            {
                ...
                if (
                    _assessmentTypeWrapper.IsBuiltIn
                    && !string.IsNullOrEmpty(_assessmentTypeWrapper.BuiltInType)
                )
                {
                    // Get the type name, which is the string representation of the enum
                    string typeName =
                        _assessmentTypeWrapper.TypeName ?? _assessmentTypeWrapper.BuiltInType;
                    ...
                    if (_assessmentTypeWrapper.JsonDefinition != null)
                    {
                        ...
                        string dataInstancePath = _assessmentTypeWrapper
                            .JsonDefinition
                            .AssessmentDataInstanceDirectory;
                        if (!string.IsNullOrEmpty(dataInstancePath))
                        {
                            // Extract the class name from the path
                            string className = System.IO.Path.GetFileNameWithoutExtension(
                                dataInstancePath
                            );

                            // Try to find the type by name
                            modelDataType = Assembly
                    ...
                    if (modelDataType != null)
                    {
                        ...
                        var typedList = genericMethod.Invoke(
                            null,
                            new object[] { jsonContent, options }
                        );
                    ...
                    var elements = JsonSerializer.Deserialize<List<JsonElement>>(
                        jsonContent,
                        options
                    );
                ...
                else if (!string.IsNullOrEmpty(_assessmentTypeWrapper.CustomTypeId))
                {
                    ...
                    return JsonSerializer
                        .Deserialize<List<Dictionary<string, JsonElement>>>(jsonContent, options)
                        .Cast<object>()
                        .ToList();
                }
